...
public class DHTSocketServer {
    ...
    private void sendTCPMessage(String message) {
        ...
        try {
            ...
            System.out.println("Next Server IP: " + this.nextServer.getHostAddress());
            //System.out.println(InetAddress.getLoopbackAddress());
            System.out.println("TCP Message sending.. " + message);
    ...
    private boolean sendUDPMessage(String message, InetAddress add) throws UnknownHostException {
        ...
        try {
            ...
            DatagramPacket sendPacket = new DatagramPacket(sendData, sendData.length, add, this.destPort);
            UDPSocket.send(sendPacket);
            System.out.println("UDP Message sent: " + message + " to port " + this.destPort);
            sent = true;
        } catch (IOException e) {
        ...
        public void run() {
            ...
            while (true) {
                try {
                    ...
                    System.out.print("Received UDP: " + message);
                    if (message.startsWith("Request for IP address") && id == 1) {
                        sendUDPMessage((id + " " + thisServer.toString()), add);
                        sendTCPMessage("GET IP" + add);
                        System.out.println("Sent IP Address to Peer and Request to next Server.");
                        System.out.println("UDP to Peer: ID " + id + " " + thisServer.toString());
                    }
        ...
        public void run() {
            ...
            while (true) {
                ...
                try {
                    ...
                    System.out.println("TCP message received: " + message);
